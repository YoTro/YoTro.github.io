...
<div>\begin{aligned} h_{{min}}=\lceil \log _{m}(n+1)\rceil -1 \end{aligned}</div>
...
<div>\begin{aligned} h_{{max}}=\left\lfloor \log _{d}{\frac {n+1}{2}}\right\rfloor \end{aligned}</div>
