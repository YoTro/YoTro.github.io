...
\begin{equation} \begin{split}  h_{{min}}=\lceil \log _{m}(n+1)\rceil -1 \end{split} \end{equation}

Let `d` be the minimum number of children an internal (non-root) node can have. 

For an ordinary B-tree,  \begin{equation} \begin{split}  d=\left\lceil m/2\right\rceil \end{split} \end{equation}

Comer (1979, p. 127) and Cormen et al. (2001, pp. 383–384) give the worst case height (the maximum height) of a B-tree as

\begin{equation} \begin{split} h_{{max}}=\left\lfloor \log _{d}{\frac {n+1}{2}}\right\rfloor \end{split} \end{equation}
