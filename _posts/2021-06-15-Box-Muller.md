...
2. 消元：U 的第 2 行就是 $\mathbf{V}^{(2)}$ 的第 2 行：
...
5. 计算 $L_{54}$，并更新 $\mathbf{V}^{(5)}$，得到 $U_{4\cdot}$。
...
3. 计算中间向量 $$\mathbf{g} = \mathbf{V}^{-1} \mathbf{m}$$
...
$$W^2 = n \int_{-\infty}^{\infty} [F_n(x) - F_0(x)]^2 dF_0(x) = \sum_{i=1}^{n} \left( U_{(i)} - \frac{2i - 1}{2n} \right)^2 + \frac{1}{12n} \text{(0)}$$
