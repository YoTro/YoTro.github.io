...
tags: [Linear algebra,python,Classification]
music-id: 
youtube-video-ID: 
...
![loading...](https://pic3.zhimg.com/80/v2-a3af0f48e0ecb8ae7a2bea3eb9fbbade_1440w.png, "FIGURE 1 A linear Support Vector Machine")

In the Machine Learing, the support vector machine is a model algorithm that defines which category an element belongs to.

We could define a super plane which is $$u = \vec{w}\cdot \vec{x} - b$$, the plane has N - 1 dimensions if the sample space is N dimensions.

We define the margin of 2 super planes between these 2 classes to classfiy them if this datas' u is bigger than 1 or less than -1  
...
=> $$L(w, b, \alpha) = \frac {1}{2}w^Tw + \sum_{i = 1}^{N}\lambda_i(1 - y_iw^Tx_i + b)$$ 
...
<=> $$\min_{\lambda}\max_{w, b}L(w, b, \lambda)$$
subject to $$\lambda_i \geq 0$$

The partial derivatives of L equal to 0

$$\frac {\partial L}{\partial b} = 0 => \sum_{i = 1}^{N}\lambda_iy_ib = 0
\\L(w,b,\lambda)=\frac {1}{2}w^Tw + \sum_{i = 1}^{N}\lambda_i - \sum_{i = 1}^{N}\lambda_i(y_iw^Tx_i + b)\\=\frac {1}{2}w^Tw + \sum_{i = 1}^{N}\lambda_i - \sum_{i = 1}^{N}\lambda_iy_iw^Tx_i$$ 
where $$\sum_{i = 1}^{N}\lambda_iy_ib = 0$$

$$\frac {\partial L}{\partial w} = 0 \\\frac {\partial (\frac {1}{2}w^Tw + \sum_{i = 1}^{N}\lambda_i - \sum_{i = 1}^{N}\lambda_iy_iw^Tx_i)}{\partial w}\\=\frac {1}{2}\cdot2w - \sum_{i = 1}^{N}\lambda_iy_ix_i\\=w  - \sum_{i = 1}^{N}\lambda_iy_ix_i = 0\\=>w = \sum_{i = 1}^{N}\lambda_iy_ix_i$$
...
$${||x - {x}'||}^{2}_{2}$$ is squre of euclidean distance
...
6. 
7. sklearn数据集文件地址: /Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/sklearn/datasets/data

##svm 数学内核的手写推导

<iframe src="//player.bilibili.com/player.html?aid=28186618&bvid=BV1Hs411w7ci&cid=58592623" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe>
...
![](https://scikit-learn.org/stable/_images/sphx_glr_plot_classifier_comparison_001.png, "classifiers comparison")
