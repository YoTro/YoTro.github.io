...
Because not all datas are linearly separable, we should introduce the [hinge loss function](https://en.m.wikipedia.org/wiki/Hinge_loss),
...
$$\begin{aligned}b &= y_i - \vec{w}\cdot \vec{x_i}\\&=y_i - \sum_{i = 1}^N\lambda_iy_i\vec{x_i}\vec{x_j} \end{aligned}$$
