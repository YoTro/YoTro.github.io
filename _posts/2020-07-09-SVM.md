...
Optimize f(x) where subject to g(x) <= 0, h(x) <= 0, x is belongs to X , we could use the Lagrange function $$L(\mathbf{x}, \mu, \lambda) = f(x) + \mu^Tg(\mathbf{x}) + \lambda^Th(\mathbf{x})$$ where $$g(\mathbf{x}) = (g_1(\mathbf{x})), ...,(g_m(\mathbf{x}))^T, h(\mathbf{x}) = (h_1(\mathbf{x})), ...,(h_l(\mathbf{x}))^T$$
...
We got the equation of `w` and `b` , which could solve by training dataset to get the λ then calculate them
