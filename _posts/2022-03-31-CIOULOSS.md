...
| **损失函数**         | **数学公式**                                                                 |
|----------------------|------------------------------------------------------------------------------|
| **VarifocalLoss**    | $$L = \alpha \cdot p^\gamma \cdot (1 - q) + q \cdot \text{BCE}(p, q)$$      |
| **FocalLoss**        | $$L = -\alpha_t (1 - p_t)^\gamma \log(p_t)$$                                |
| **DFLoss**           | $$L = \text{CrossEntropy}(p, y_l) \cdot w_l + \text{CrossEntropy}(p, y_r) \cdot w_r$$ |
| **BboxLoss**         | $$\begin{align}\text{FG 表示所有正样本的集合}\\w_i &= \sum_j \text{target_scores}_{ij}\\S&=target_scores_sum 是正样本目标得分的总和\\\text{IoU 损失：}L_{iou} & = \frac{1}{S} \sum_{i \in \text{FG}} w_i \cdot \left(1 - IoU(\hat{b}_i, b_i)\right) \text{   if reg_max}\le 1\\\text{DFL 损失：}L_{dfl} & = \frac{1}{S} \sum_{i \in \text{FG}} w_i \cdot \text{DFLoss}(\text{pred_dist}_i, t_i) \text{   if reg_max}>1\end{align}$$ |
...
| **KeypointLoss**     | $$\begin{cases}d_{i,k} &= \left(x_{i,k}^{\text{pred}} - x_{i,k}^{\text{gt}}\right)^2 + \left(y_{i,k}^{\text{pred}} - y_{i,k}^{\text{gt}}\right)^2, \text{对于每个样本 i 中的每个关键点 k}\\e_{i,k} &= \frac{d_{i,k}}{8\,\sigma_k^2 \cdot (A_i + \epsilon)}, A_i=\text{目标区域面积​ }\sigma_k=\text{标准差}\\l_{i,k} &= 1 - e^\left(-e_{i,k}\right),  \text{}\\\lambda_i &= \frac{K}{\sum_{k=1}^{K} m_{i,k} + \epsilon},  K=\text{关键点总数(即 kpt_mask.shape[1]),}m_{i,k}=\text{关键点掩码，若该关键点有效则为 1，否则为 0）}\\L &= \frac{1}{N \times K} \sum_{i=1}^{N} \lambda_i \sum_{k=1}^{K} m_{i,k}\cdot l_{i,k}， \text{N 是批次中的样本数}\end{cases}$$ |
| **v8DetectionLoss**  | $$L = L_{cls} + L_{box} + L_{dfl}$$                   |
| **v8SegmentationLoss** | $$L = L_{box} + L_{cls} + L_{dfl} + L_{mask}$$ |
| **v8PoseLoss**       | $$\begin{cases}L_{\text{box}} = \frac{1}{S} \sum_{i\in\mathcal{F}} w_i \Bigl[1 - IoU(\hat{b}_i, b_i)\Bigr], \quad L_{\text{dfl}} = \frac{1}{S} \sum_{i\in\mathcal{F}} w_i \cdot \text{DFLoss}(p_i, t_i)\\L_{\text{cls}} = \frac{1}{S} \sum_{i} \text{BCE}(p^{cls}_i, q_i)\\ L_{\text{kpt}} = \frac{1}{N\cdot K} \sum_{i=1}^{N} \lambda_i \sum_{k=1}^{K} m_{i,k}\, \left[1 - \exp\Bigl(-\frac{d_{i,k}}{8\,\sigma_k^2\,(A_i+\epsilon)}\Bigr)\right] \\L_{\text{kobj}} = \text{BCE_pose}\Bigl(p^{\text{kobj}}_{i,k}, m_{i,k}\Bigr)\\\mathcal{L} = N \cdot \Bigl[ \lambda_{\text{box}} L_{\text{box}} + \lambda_{\text{pose}} L_{\text{kpt}} + \lambda_{\text{kobj}} L_{\text{kobj}} + \lambda_{\text{cls}} L_{\text{cls}} + \lambda_{\text{dfl}} L_{\text{dfl}} \Bigr],\lambda_{box​}=self.hyp.box\end{cases}$$ |
| **v8ClassificationLoss** | $$L = -\sum y \log(p)$$                                                   |
| **v8OBBLoss**        | $$L = L_{box} + L_{cls} + L_{dfl}$$                  |
| **E2EDetectLoss**    | $$L = L_{one2many} + L_{one2one}$$                          |
