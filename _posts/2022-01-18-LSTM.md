...
1. **遗忘门（Forget Gate）**  
   ...
   - 输出：一个值在 (0) 到 (1) 之间的向量，表示每个单元的遗忘程度。
...
2. **输入门（Input Gate）**  
   ...
   - 包含两部分：
     - 更新信息的候选值。
     - 选择需要更新的部分。
   - 公式：$$i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i)$$$$\tilde{C}_t = \tanh(W_c \cdot [h_{t-1}, x_t] + b_c)$$
...
$$\left\{\begin{matrix}
 f_t = \sigma_g(W_fx_t+U_fh_{t-1}+b_f) & (遗忘门)\\
 i_t = \sigma_g(W_ix_t+U_ih_{t-1}+b_t) & (输入门)\\
 o_t = \sigma_g(W_ox_t+U_oh_{t-1}+b_o) & (输出门)\\
 \tilde{c}_t = \sigma_c(W_cx_t+U_ch_{t-1}+b_c) & (候选单元状态)\\
 c_t = f_t \odot c_{t-1} + i_t \odot \tilde{c}_t & (单元状态更新)\\
 h_t = o_t \odot \sigma_h(c_t) & (隐藏状态更新)
\end{matrix}\right.$$
...
- $$o_t$$: 第t时刻的输出门的输出。
- $$h_t$$: 第t时刻的隐藏状态。
- $$\sigma$$: 激活函数（如tanh 或 RELU), 其中下标是用来区别在不同计算中使用的激活函数类型。
- $$b$$: 偏置。
- $$f_t \odot c_{t-1}$$: t时刻遗忘门控制对于过去状态t-1的保留程度。
- $$i_t \odot \tilde{c}_t$$: t时刻输入门控制对于当前候选状态的采纳程度。
...
$$h_t = \sigma(W_tx_t + U_hh_{t-1} + b_h)$$


# Code精读
[LSTM pytorch官方实现](https://github.com/pytorch/pytorch/blob/main/torch/nn/modules/rnn.py#L795)
```python
class LSTM(RNNBase):
