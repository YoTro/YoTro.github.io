...
\text{假设，} \quad \sigma &= \left\{\begin{matrix}
\tanh(x) \\
 \sigma(x)
\end{matrix} \right. \\
\text{则令，} \quad z_t &= W_t x_t + W_h h_{t-1} + b_h \\
\Rightarrow \sigma'(z_t) &= \left\{
\begin{matrix}
  1 - \tanh^2(z_t) \\
 \sigma(z_t) \odot (1 - \sigma(z_t))
\end{matrix} \right. \\
\Rightarrow \frac{\partial h_t}{\partial W_{hh}} &= \frac{\partial \sigma(z_t)}{\partial z_t} \frac{\partial z_t}{\partial W_{hh}} = (1 - \tanh^2(z_t)) h_{t-1}^\top \tag{1} \\
\text{其中，} \quad \frac{\partial L}{\partial h_{t}} &= \frac{\partial L}{\partial h_{T}} \prod_{k=t+1}^{T} \frac{\partial h_{k}}{\partial h_{k-1}} \\
&= \frac{\partial L}{\partial h_{T}} \prod_{k=t+1}^{T} \frac{\partial h_{k}}{\partial z_{k}} \frac{\partial z_{k}}{\partial h_{k-1}} \\
&= \frac{\partial L}{\partial h_{T}} \prod_{k=t+1}^{T} (1 - \tanh^2(z_k)) W_{hh} \tag{2} \\
\text{(1) + (2)} \Rightarrow \frac{\partial L}{\partial W_{hh}} &= \sum_{t=1}^{T} \left( \frac{\partial L}{\partial h_{T}} \prod_{k=t+1}^{T} (1 - \tanh^2(z_k)) W_{hh} (1 - \tanh^2(z_t)) h_{t-1}^\top \right)
\end{align}$$
...
2. 实验 2 关注于无噪声和有噪声的序列，这些序列包含大量分散注意力的输入符号，而只有少数是重要的。最困难的任务（任务 2c）涉及在随机位置出现的数百个干扰符号，以及最小时间间隔为 1000 步。
3. 实验 3 旨在解决噪声和信号位于同一输入线上的长时间延迟问题。实验 3a 和 3b 主要关注 Bengio 等人于 1994 年提出的“2-序列问题”。由于该问题实际上可以通过随机权重猜测快速解决，因此作者还引入了一个更具挑战性的 2-序列问题（实验 3c），该问题要求模型在给定输入的情况下学习噪声目标的实值条件期望。 
4. 
# 效果
...
![Long Short-Term Memory](/images/MachineLearing/LSTM/LSTM.png)
...
\nabla g_o&=\frac{\partial L}{\partial h_t}\odot tanh(c_t) \tag{1}
\end{align}$$
...
=\nabla g_o\odot\sigma \odot (1-\sigma )\odot x^\top_t & \text{(8)}\\
...
=\nabla g_g \odot(1-\tanh^2)\odot h^\top_{t-1} & \text{(11)}
\end{matrix}\right.$$
...
=\nabla g_f\odot \sigma \odot (1-\sigma )\odot x^\top_t & \text{(12)}\\
...
=\nabla g_f\odot \sigma \odot (1-\sigma )\odot h^\top_{t-1} & \text{(13)}
\end{matrix}\right.$$
...
=\nabla g_i \odot \sigma \odot (1-\sigma )\odot x^\top_t & \text{(14)}\\
...
=\nabla g_i \odot \sigma \odot (1-\sigma )\odot h^\top_{t-1} & \text{(15)}
\end{matrix}\right.$$
