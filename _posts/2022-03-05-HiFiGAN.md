...
2. 梅尔频谱损失(Mel-Spectrogram Loss): ![](http://latex.codecogs.com/gif.latex?
\mathcal L_{Mel}(G))，将生成音频转换回梅尔频谱之后，计算真实和生成梅尔频谱之间的L1距离；
3. 特征匹配损失(Feature Match Loss): ![](http://latex.codecogs.com/gif.latex?\mathcal L_{FM} (G; D))，主要是对比中间卷积层真实和合成样本之间的差异。
